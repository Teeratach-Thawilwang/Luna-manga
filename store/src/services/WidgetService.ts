...
} from "@interfaces/WidgetInterface";
import WidgetApi from "@repositories/WidgetApi";
import AuthService from "@services/AuthService";
import CookieService from "@services/CookieService";
import store, { useAppSelector } from "@store/Store";
import { pushWidgets, update, updateWidgetStateById } from "@store/slices/WidgetSlice";
...
class WidgetService {
  ...
  public getState(): WidgetSliceInterface {
    return useAppSelector((state) => state.widget, shallowEqual);
  }

  public getWidgets(): WidgetStateInterface[] {
    return useAppSelector((state) => state.widget.data, shallowEqual);
  }

  public getIsLoading(): boolean {
    return useAppSelector((state) => state.widget.is_loading, shallowEqual);
  }

  public getIsLoaded(): boolean {
    return useAppSelector((state) => state.widget.paginate != null, shallowEqual);
  }

  public getCurrentPage(): number {
    return useAppSelector((state) => state.widget.paginate?.current ?? 1, shallowEqual);
  }

  public getLastPage(): number | null | undefined {
    return useAppSelector((state) => state.widget.paginate?.last, shallowEqual);
  }

  public getError(): string {
    return useAppSelector((state) => state.widget.error, shallowEqual);
  }

  public getPaginateById(id: number): PaginationInterface | null {
    const widget = useAppSelector((state) => {
      const widget = state.widget.data.filter((widget) => widget.data.id === id)[0] ?? null;
