...
import { CategoryIndexParams, CategoryInterface, CategorySliceInterface } from "@interfaces/CategoryInterface";
...
import { pushCategories, update } from "@store/slices/CategorySlice";
...
class CategoryService {
  ...
  public getState(): CategorySliceInterface {
    return useAppSelector((state) => state.category, shallowEqual);
  }

  public getCategory(): CategoryInterface[] {
    return useAppSelector((state) => state.category.data, shallowEqual);
  }

  public getCategoryType(): CategoryTypeEnum {
    return useAppSelector((state) => state.category.type ?? CategoryTypeEnum.MANGA, shallowEqual);
  }

  public getIsLoading(): boolean {
    return useAppSelector((state) => state.category.is_loading, shallowEqual);
  }

  public getIsLoaded(): boolean {
    return useAppSelector((state) => state.category.paginate != null, shallowEqual);
  }

  public getCategoryCurrentPage(): number {
    return useAppSelector((state) => state.category.paginate?.current ?? 1, shallowEqual);
  }

  public getCategoryError(): string {
    return useAppSelector((state) => state.category.error, shallowEqual);
  }
