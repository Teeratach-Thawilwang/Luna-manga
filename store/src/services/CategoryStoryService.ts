...
import { CategoryStoryIndexParams, CategoryStoryInterface, CategoryStorySliceInterface } from "@interfaces/CategoryStoryInterface";
import { PaginationInterface } from "@interfaces/PaginationInterface";
import { AxiosResponseError } from "@interfaces/ResponseErrorInterface";
import CategoryStoryApi from "@repositories/CategoryStoryApi";
import AuthService from "@services/AuthService";
import store, { useAppSelector } from "@store/Store";
import { pushStories, update } from "@store/slices/CategoryStorySlice";
...
class CategoryStoryService {
  ...
  public getState(): CategoryStorySliceInterface {
    return useAppSelector((state) => state.categoryStory, shallowEqual);
  }

  public getStory(): CategoryStoryInterface[] {
    return useAppSelector((state) => state.categoryStory.data, shallowEqual);
  }

  public getCategorySelectedId(): number | null {
    return useAppSelector((state) => state.categoryStory.category_selected_id, shallowEqual);
  }

  public getIsLoading(): boolean {
    return useAppSelector((state) => state.categoryStory.is_loading, shallowEqual);
  }

  public getIsLoaded(): boolean {
    return useAppSelector((state) => state.categoryStory.paginate != null, shallowEqual);
  }

  public getCurrentPage(): number {
    return useAppSelector((state) => state.categoryStory.paginate?.current ?? 1, shallowEqual);
  }

  public getPaginate(): PaginationInterface | null {
    return useAppSelector((state) => state.categoryStory.paginate, shallowEqual);
  }

  public getError(): string {
    return useAppSelector((state) => state.categoryStory.error, shallowEqual);
  }
