...
} from "@interfaces/ProfilePostInterface";
import { AxiosResponseError } from "@interfaces/ResponseErrorInterface";
import ProfilePostApi from "@repositories/ProfilePostApi";
import AuthService from "@services/AuthService";
import store, { useAppSelector } from "@store/Store";
import { deletePostById, pushData, pushDataAtBegin, update, updateReaction } from "@store/slices/ProfilePostSlice";
...
class ProfilePostService {
  ...
  public getState(): ProfilePostSliceInterface {
    return useAppSelector((state) => state.profilePost, shallowEqual);
  }

  public getPosts(): ProfilePostInterface[] {
    return useAppSelector((state) => state.profilePost.data, shallowEqual);
  }

  public getPostCount(): number {
    return useAppSelector((state) => state.profilePost.paginate?.total ?? 0, shallowEqual);
  }

  public getPostIsLoading(): boolean {
    return useAppSelector((state) => state.profilePost.is_loading, shallowEqual);
  }

  public getPostIsLoaded(): boolean {
    return useAppSelector((state) => state.profilePost.paginate != null, shallowEqual);
  }

  public getPostPaginate(): PaginationInterface | null {
    return useAppSelector((state) => state.profilePost.paginate, shallowEqual);
  }

  public getPostCurrentPage(): number {
    return useAppSelector((state) => state.profilePost.paginate?.current ?? 1, shallowEqual);
  }

  public getPostError(): string {
    return useAppSelector((state) => state.profilePost.error, shallowEqual);
  }
