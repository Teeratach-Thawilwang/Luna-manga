...
import ReportPostConfirmModal from "@components/frontside/tablet/profile/post/ReportPostConfirmModal";
...
export default function ReportPostModal({ isShow, postId, posterId, setIsShow }: ReportPostModalInterface) {
  ...
  const isOnwer = getIsOwner(customerId, posterId);
  ...
  return (
    <>
      ...
      <CenterModal isShow={state.isShow} setShow={setShowHandle}>
        <ReportPostConfirmModal group={state.group} text={state.text} customerId={customerId!} postId={postId} setShow={setShowHandle} />
      </CenterModal>
    </>
  );
}
...
function getIsOwner(customerId: number | null, posterId: number): boolean {
  if (customerId != null && customerId == posterId) {
    return true;
  }
  return false;
}
