...
export default function InputName({ initial }: { initial?: string }) {
  ...
  const errorMessage = WidgetCreateEditService.getter<string>("name_error_message");
  const errorMessageElement = getErrorMessageElement(errorMessage);

  useEffect(() => {
    WidgetCreateEditService.update({ name: name });
  }, [nameDebounce]);
  ...
  return (
    <Box>
      <Title> Name</Title>
      <Input maxLength={50} type="text" placeholder="ชื่อ Widget" value={name} onChange={onChangeHandle} />
      {errorMessageElement}
    </Box>
  );
}
