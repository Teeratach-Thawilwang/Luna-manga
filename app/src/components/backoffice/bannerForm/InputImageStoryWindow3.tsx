...
export default function InputImageStoryWindow3() {
  const imagesState = BannerCreateEditService.getter<BannerImageInterface[] | ImageInterface[]>("images");
  const initialImage = BannerCreateEditService.getImageByCollection(CollectionEnum.BANNER_STORY_WINDOW_3) as ImageInterface;
  ...
  return (
    <Box>
      <ImageBox htmlFor="input-image-story-window-3">{image}</ImageBox>
      <Input type="file" accept="image/*" id="input-image-story-window-3" onChange={onChangeHandle} />
    </Box>
  );
}
...
function createImageElement(url: string) {
  if (url != "") {
    return (
      <>
        <Image src={url} />
        <PlaceHolder>980 x 470 Pixels</PlaceHolder>
      </>
    );
  }
...
function transformImages(images: BannerImageInterface[] | ImageInterface[], file: File) {
  const isImageExist = images.some((image) => image.collection_name == CollectionEnum.BANNER_STORY_WINDOW_3);
  const uploadImage = {
    file: file,
    collection_name: CollectionEnum.BANNER_STORY_WINDOW_3,
  };
  ...
  return images.map((image) => {
    if (image.collection_name == CollectionEnum.BANNER_STORY_WINDOW_3) {
      return uploadImage;
    }
    return image;
  });
