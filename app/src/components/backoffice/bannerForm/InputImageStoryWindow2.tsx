...
export default function InputImageStoryWindow2() {
  const imagesState = BannerCreateEditService.getter<BannerImageInterface[] | ImageInterface[]>("images");
  const initialImage = BannerCreateEditService.getImageByCollection(CollectionEnum.BANNER_STORY_WINDOW_2) as ImageInterface;
  ...
  return (
    <Box>
      <ImageBox htmlFor="input-image-story-window-2">{image}</ImageBox>
      <Input type="file" accept="image/*" id="input-image-story-window-2" onChange={onChangeHandle} />
    </Box>
  );
}
...
function transformImages(images: BannerImageInterface[] | ImageInterface[], file: File) {
  const isImageExist = images.some((image) => image.collection_name == CollectionEnum.BANNER_STORY_WINDOW_2);
  const uploadImage = {
    file: file,
    collection_name: CollectionEnum.BANNER_STORY_WINDOW_2,
  };
  ...
  return images.map((image) => {
    if (image.collection_name == CollectionEnum.BANNER_STORY_WINDOW_2) {
      return uploadImage;
    }
    return image;
  });
