...
export default function InputName({ initial }: { initial?: string }) {
  ...
  const errorMessage = BannerCreateEditService.getter<string>("name_error_message");
  const errorMessageElement = getErrorMessageElement(errorMessage);

  useEffect(() => {
    BannerCreateEditService.update({ name: name });
  }, [nameDebounce]);
  ...
  return (
    <Box>
      <Title> Name</Title>
      <Input maxLength={50} type="text" placeholder="ชื่อ Banner" value={name} onChange={onChangeHandle} />
      {errorMessageElement}
    </Box>
  );
}
