...
export default function InputTitle({ initial }: { initial?: string }) {
  ...
  const errorMessage = BannerCreateEditService.getter<string>("title_error_message");
  const errorMessageElement = getErrorMessageElement(errorMessage);

  useEffect(() => {
    BannerCreateEditService.update({ title: title });
  }, [nameDebounce]);
  ...
  return (
    <Box>
      <Title> Title</Title>
      <Input maxLength={50} type="text" placeholder="ชื่อ Banner ที่ใช้แสดงบนหน้าเว็ป" value={title} onChange={onChangeHandle} />
      {errorMessageElement}
    </Box>
  );
}
