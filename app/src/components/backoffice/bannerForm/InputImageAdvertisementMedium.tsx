...
export default function InputImageAdvertisementMedium() {
  const imagesState = BannerCreateEditService.getter<BannerImageInterface[] | ImageInterface[]>("images");
  const initialImage = BannerCreateEditService.getImageByCollection(CollectionEnum.BANNER_ADVERTISEMENT_MEDIUM) as ImageInterface;
  ...
  return (
    <Box>
      <ImageBox htmlFor="input-image-advertisement-medium">{image}</ImageBox>
      <Input type="file" accept="image/*" id="input-image-advertisement-medium" onChange={onChangeHandle} />
    </Box>
  );
}
...
function transformImages(images: BannerImageInterface[] | ImageInterface[], file: File) {
  const isImageExist = images.some((image) => image.collection_name == CollectionEnum.BANNER_ADVERTISEMENT_MEDIUM);
  const uploadImage = {
    file: file,
    collection_name: CollectionEnum.BANNER_ADVERTISEMENT_MEDIUM,
  };
  ...
  return images.map((image) => {
    if (image.collection_name == CollectionEnum.BANNER_ADVERTISEMENT_MEDIUM) {
      return uploadImage;
    }
    return image;
  });
