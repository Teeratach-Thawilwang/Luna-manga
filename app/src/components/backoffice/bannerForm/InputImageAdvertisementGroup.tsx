...
export default function InputImageAdvertisementGroup() {
  const imagesState = BannerCreateEditService.getter<BannerImageInterface[] | ImageInterface[]>("images");
  const initialImage = BannerCreateEditService.getImageByCollection(CollectionEnum.BANNER_ADVERTISEMENT_GROUP) as ImageInterface;
  ...
  return (
    <Box>
      <ImageBox htmlFor="input-image-advertisement-group">{image}</ImageBox>
      <Input type="file" accept="image/*" id="input-image-advertisement-group" onChange={onChangeHandle} />
    </Box>
  );
}
...
function createImageElement(url: string) {
  if (url != "") {
    return (
      <>
        <Image src={url} />
        <PlaceHolder>1000 x 180 Pixels</PlaceHolder>
      </>
    );
  }
...
function transformImages(images: BannerImageInterface[] | ImageInterface[], file: File) {
  const isImageExist = images.some((image) => image.collection_name == CollectionEnum.BANNER_ADVERTISEMENT_GROUP);
  const uploadImage = {
    file: file,
    collection_name: CollectionEnum.BANNER_ADVERTISEMENT_GROUP,
  };
  ...
  return images.map((image) => {
    if (image.collection_name == CollectionEnum.BANNER_ADVERTISEMENT_GROUP) {
      return uploadImage;
    }
    return image;
  });
