...
export default function ChapterSetting({ groupSetting }: { groupSetting: GroupSettingEnum }) {
  // console.log("In ChapterSetting");
  const [isDropdownOpen, setIsDropdownOpen] = useState(true);
  const isMatchMangaUrl = groupSetting == GroupSettingEnum.CHAPTER_MANGA;
  const isMatchNovelUrl = groupSetting == GroupSettingEnum.CHAPTER_NOVEL;
  const isMainButtonActive = isMatchMangaUrl || isMatchNovelUrl;
  const userPermissions = UserProfileService.getPermissions()!;
  const hasChapterPermission = hasPermission(PermissionEnum.CHAPTER_VIEW, userPermissions);

  if (!hasChapterPermission) {
    return null;
  }
  ...
  return (
    <Box $isOpen={isDropdownOpen}>
      ...
      <ItemList $isOpen={isDropdownOpen}>
        <Item to={GroupSettingUrlEnum.CHAPTER_MANGA} $isOpen={isDropdownOpen} $isActive={isMatchMangaUrl}>
          <span>• {GroupSettingNameEnum.CHAPTER_MANGA}</span>
        </Item>
        <Item to={GroupSettingUrlEnum.CHAPTER_NOVEL} $isOpen={isDropdownOpen} $isActive={isMatchNovelUrl}>
          <span>• {GroupSettingNameEnum.CHAPTER_NOVEL}</span>
        </Item>
      </ItemList>
    </Box>
  );
