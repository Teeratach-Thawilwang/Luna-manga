...
export default function FilterSource() {
  ...
  function onDropDownHandle(source: CustomerReportSourceEnum | null) {
    CustomerReportListService.updateFilter({ source: source });
    setIsOpen(false);
  ...
  return (
    <Box ref={dropDownRef}>
      <DropdownButton onClick={() => setIsOpen((prev) => !prev)}>
        <span>Source: {source != null ? transformFirstCapital(source) : "All"}</span>
      ...
      <DropdownContent $isOpen={isOpen}>
        ...
        <DropdownItem onClick={() => onDropDownHandle(CustomerReportSourceEnum.STORY)}>
          {transformFirstCapital(CustomerReportSourceEnum.STORY)}
        </DropdownItem>
        <DropdownItem onClick={() => onDropDownHandle(CustomerReportSourceEnum.CHAPTER)}>
          {transformFirstCapital(CustomerReportSourceEnum.CHAPTER)}
        </DropdownItem>
        <DropdownItem onClick={() => onDropDownHandle(CustomerReportSourceEnum.POST)}>
          {transformFirstCapital(CustomerReportSourceEnum.POST)}
        </DropdownItem>
        <DropdownItem onClick={() => onDropDownHandle(CustomerReportSourceEnum.COMMENT)}>
          {transformFirstCapital(CustomerReportSourceEnum.COMMENT)}
        </DropdownItem>
      </DropdownContent>
    </Box>
  );
