...
import CustomerReportListPagination from "@components/backoffice/customerReportList/CustomerReportListPagination";
import CustomerReportListService from "@services/backoffice/CustomerReportListService";
import { transformEnumToWord, transformFirstCapital, transfromDateString } from "@utils/Helpers";

export default function CustomerReportTable() {
  const navigate = useNavigate();
  const customerReports = CustomerReportListService.getData();
  const customerReportFilter = CustomerReportListService.getFilter();
  const isLoading = CustomerReportListService.getIsLoading();
      ...
      <Table $isLoading={isLoading}>
        ...
        <Tbody>
          {customerReports.map((customerReport) => (
            <tr key={customerReport.id} onClick={() => navigate(`/backoffice/customer-report/${customerReport.id}`)}>
              <Td>{customerReport.id}</Td>
              <Td>{transformEnumToWord(customerReport.group)}</Td>
              <Td>{transformFirstCapital(customerReport.source)}</Td>
              <Td>{customerReport.customer_id}</Td>
              <Td>
                <StatusBox $bgcolor={customerReport.is_accept ? "#3BA639" : "#ADADAD"}>{customerReport.is_accept ? "True" : "False"}</StatusBox>
              </Td>
              <Td>{transfromDateString(customerReport.created_at)}</Td>
              <Td>{transfromDateString(customerReport.updated_at)}</Td>
            </tr>
          ))}
        </Tbody>
      </Table>
      <CustomerReportListPagination navigatePagination={navigatePagination} />
    </Box>
  );
}
