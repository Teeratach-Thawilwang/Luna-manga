...
export default function FilterStartDate() {
  ...
  const startDate = CustomerReportListService.getStartDate();
  const endDate = CustomerReportListService.getEndDate();
  const boxRef: MutableRefObject<HTMLDivElement | null> = useRef(null);

  function handleChange(date: Date | null) {
    CustomerReportListService.updateFilter({ start_date: String(date) });
  ...
  return (
    <Box ref={boxRef} onClick={onClickHandle}>
      ...
      />
      <IconBox onClick={() => CustomerReportListService.updateFilter({ start_date: null })}>
        <CalendarIcon />
      </IconBox>
    </Box>
  );
