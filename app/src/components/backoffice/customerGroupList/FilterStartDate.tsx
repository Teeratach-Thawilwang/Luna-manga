...
export default function FilterStartDate() {
  ...
  const startDate = CustomerGroupListService.getStartDate();
  const endDate = CustomerGroupListService.getEndDate();
  const boxRef: MutableRefObject<HTMLDivElement | null> = useRef(null);

  function handleChange(date: Date | null) {
    CustomerGroupListService.updateFilter({ start_date: String(date) });
  ...
  return (
    <Box ref={boxRef} onClick={onClickHandle}>
      ...
      />
      <IconBox onClick={() => CustomerGroupListService.updateFilter({ start_date: null })}>
        <CalendarIcon />
      </IconBox>
    </Box>
  );
