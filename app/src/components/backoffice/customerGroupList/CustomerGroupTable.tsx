...
import CustomerGroupListPagination from "@components/backoffice/customerGroupList/CustomerGroupListPagination";
...
export default function CustomerGroupTable() {
  ...
  const customerGroups = CustomerGroupListService.getData();
  const customerGroupFilter = CustomerGroupListService.getFilter();
  const isLoading = CustomerGroupListService.getIsLoading();
      ...
      <Table $isLoading={isLoading}>
        ...
        <Tbody>
          {customerGroups.map((customerGroup) => (
            <tr key={customerGroup.id} onClick={() => navigate(`/backoffice/customer-group/${customerGroup.id}`)}>
              ...
              <Td>{transfromDateString(customerGroup.created_at)}</Td>
              <Td>{transfromDateString(customerGroup.updated_at)}</Td>
            </tr>
          ))}
        </Tbody>
      </Table>
      <CustomerGroupListPagination navigatePagination={navigatePagination} />
    </Box>
  );
}
