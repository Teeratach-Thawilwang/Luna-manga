...
export default function InputStatus({ initial }: { initial?: CategoryStatusEnum }) {
  const inActiveRef = useRef<HTMLInputElement>(null);
  const activeRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (initial == CategoryStatusEnum.ACTIVE) {
      CategoryCreateEditService.update({ status: CategoryStatusEnum.ACTIVE });
      activeRef.current!.checked = true;
    }
    if (initial == CategoryStatusEnum.INACTIVE || !initial) {
      CategoryCreateEditService.update({ status: CategoryStatusEnum.INACTIVE });
  ...
  return (
    <Box>
      ...
      <CheckBox onClick={() => onCheckBoxClick(inActiveRef)}>
        <input type="checkbox" name={CategoryStatusEnum.INACTIVE} ref={inActiveRef} />
        <Item>Inactive</Item>
      </CheckBox>
      <CheckBox onClick={() => onCheckBoxClick(activeRef)}>
        <input type="checkbox" name={CategoryStatusEnum.ACTIVE} ref={activeRef} />
        <Item>Active</Item>
      </CheckBox>
    </Box>
  );
