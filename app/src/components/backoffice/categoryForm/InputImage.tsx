...
export default function InputCoverImage({ initial }: { initial?: ImageInterface }) {
  ...
  const errorMessage = CategoryCreateEditService.getter<string>("image_error_message");
  const errorMessageElement = createErrorMessageElement(errorMessage);

  useEffect(() => {
    if (initial) {
      CategoryCreateEditService.update({ image: initial });
    }
  }, []);
  ...
  function onChangeHandle(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!ValidationService.validateFileSize(files[0], LimitFileSizeEnum.CATEGORY_IMAGE_SIZE)) {
      alert(`ไฟล์ต้องมีขนาดไม่เกิน ${LimitFileSizeEnum.CATEGORY_IMAGE_SIZE}MB`);
      return;
    }
    ...
    CategoryCreateEditService.update({ image: imageFile });
    event.target.value = "";
  }
  ...
  return (
    <Box>
      <Title>Image</Title>
      <Warp>
        <ImageBox htmlFor="input-category-image" width={url == "" ? "80px" : "400px"}>
          {image}
        </ImageBox>
        <Input type="file" accept="image/*" id="input-category-image" onChange={onChangeHandle} />
      </Warp>
      {errorMessageElement}
    </Box>
  );
