...
export default function FilterStartDate() {
  ...
  const startDate = CategoryListService.getStartDate();
  const endDate = CategoryListService.getEndDate();
  const boxRef: MutableRefObject<HTMLDivElement | null> = useRef(null);

  function handleChange(date: Date | null) {
    CategoryListService.updateFilter({ start_date: String(date) });
  ...
  return (
    <Box ref={boxRef} onClick={onClickHandle}>
      ...
      />
      <IconBox onClick={() => CategoryListService.updateFilter({ start_date: null })}>
        <CalendarIcon />
      </IconBox>
    </Box>
  );
