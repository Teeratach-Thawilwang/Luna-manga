...
export default function InputName({ initial }: { initial?: string }) {
  ...
  const errorMessage = StoryCreateEditService.getter<string>("name_error_message");
  const errorMessageElement = getErrorMessageElement(errorMessage);

  useEffect(() => {
    StoryCreateEditService.update({ name: name });
  }, [nameDebounce]);
  ...
  return (
    <Box>
      <Title>Name</Title>
      <Input maxLength={50} type="text" placeholder="กรอกชื่อเรื่อง ไม่เกิน 50 ตัวอักษร" value={name} onChange={onChangeHandle} />
      {errorMessageElement}
    </Box>
  );
}
