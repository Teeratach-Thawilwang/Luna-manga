...
export default function FilterStatus() {
  ...
  const status = UserListService.getStatus();
  ...
  function onDropDownHandle(status: UserStatusEnum | null) {
    UserListService.updateFilter({ status: status });
    setIsOpen(false);
    UserListService.loadUserList(userFilter?.q, status ?? undefined, userFilter?.start_date, userFilter?.end_date, 1, userFilter?.order_by);
  }
  ...
  return (
    <Box ref={dropDownRef}>
      ...
      <DropdownContent $isOpen={isOpen}>
        ...
        <DropdownItem onClick={() => onDropDownHandle(UserStatusEnum.ACTIVE)}>Active</DropdownItem>
        <DropdownItem onClick={() => onDropDownHandle(UserStatusEnum.INACTIVE)}>Inactive</DropdownItem>
      </DropdownContent>
    </Box>
  );
}
