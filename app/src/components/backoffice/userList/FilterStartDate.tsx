...
export default function FilterStartDate() {
  ...
  function handleChange(date: Date | null) {
    UserListService.updateFilter({ start_date: String(date) });
    UserListService.loadUserList(userFilter?.q, userFilter?.status, String(date), userFilter?.end_date, 1, userFilter?.order_by);
  }
  ...
  return (
    <Box ref={boxRef} onClick={onClickHandle}>
      ...
      />
      <IconBox onClick={() => UserListService.updateFilter({ start_date: null })}>
        <CalendarIcon />
      </IconBox>
    </Box>
  );
