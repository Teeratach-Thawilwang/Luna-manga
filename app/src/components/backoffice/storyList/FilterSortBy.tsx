...
export default function FilterSortBy() {
  ...
  function onDropDownHandle(sort: SortByEnum | null) {
    ...
    StoryListService.updateFilter({ order_by: orderBy });
    StoryListService.loadStoryList(storyFilter?.q, storyFilter?.status, storyFilter?.start_date, storyFilter?.end_date, type, 1, orderBy);
    setIsOpen(false);
  }
  ...
  return (
    <Box ref={dropDownRef}>
      <DropdownButton onClick={() => setIsOpen((prev) => !prev)}>
        <span>Sort by: {storyFilter?.order_by == "id" ? SortByEnum.OLDEST : SortByEnum.LATEST}</span>
