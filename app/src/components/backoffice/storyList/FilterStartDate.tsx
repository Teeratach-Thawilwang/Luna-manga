...
export default function FilterStartDate() {
  ...
  function handleChange(date: Date | null) {
    StoryListService.updateFilter({ start_date: String(date) });
    StoryListService.loadStoryList(storyFilter?.q, storyFilter?.status, String(date), storyFilter?.end_date, type, 1, storyFilter?.order_by);
  }
  ...
  return (
    <Box ref={boxRef} onClick={onClickHandle}>
      ...
      />
      <IconBox onClick={() => StoryListService.updateFilter({ start_date: null })}>
        <CalendarIcon />
      </IconBox>
    </Box>
  );
