...
import WidgetListPagination from "@components/backoffice/widgetList/WidgetListPagination";
import WidgetListService from "@services/backoffice/WidgetListService";
import { transformBgColorStatus, transformStatus, transformWidgetType, transfromDateString } from "@utils/Helpers";

export default function WidgetTable() {
  const navigate = useNavigate();
  const widgets = WidgetListService.getData();
  const widgetFilter = WidgetListService.getFilter();
  const isLoading = WidgetListService.getIsLoading();
  ...
  return (
    <Box ref={tableRef}>
      ...
      <Table $isLoading={isLoading}>
        ...
        <Tbody>
          {widgets.map((widget) => (
            <tr key={widget.id} onClick={() => navigate(`/backoffice/widget/${widget.id}`)}>
              <Td>{widget.id}</Td>
              <Td>{widget.name}</Td>
              ...
              <Td>{transfromDateString(widget.created_at)}</Td>
              <Td>{transfromDateString(widget.updated_at)}</Td>
            </tr>
          ))}
        </Tbody>
      </Table>
      <WidgetListPagination navigatePagination={navigatePagination} />
    </Box>
  );
}
