...
export default function FilterStartDate() {
  ...
  const startDate = WidgetListService.getStartDate();
  const endDate = WidgetListService.getEndDate();
  const boxRef: MutableRefObject<HTMLDivElement | null> = useRef(null);

  function handleChange(date: Date | null) {
    WidgetListService.updateFilter({ start_date: String(date) });
  ...
  return (
    <Box ref={boxRef} onClick={onClickHandle}>
      ...
      />
      <IconBox onClick={() => WidgetListService.updateFilter({ start_date: null })}>
        <CalendarIcon />
      </IconBox>
    </Box>
  );
