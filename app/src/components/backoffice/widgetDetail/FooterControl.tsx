...
import ConfirmDeleteModal from "@components/backoffice/widgetDetail/ConfirmDeleteModal";
import WidgetCreateEditService from "@services/backoffice/WidgetCreateEditService";
import { createValidateParams, isValid } from "@utils/WidgetCreateUpdateHelpers";

export default function FooterControl() {
  const { id } = useParams();
  const state = WidgetCreateEditService.getState();
  ...
  async function onUpdate() {
    ...
    WidgetCreateEditService.update({ ...validateParams });
    ...
    setPercentBar(100);
    try {
      const bannerIds = state.banners.map((banner) => banner.id);
      WidgetCreateEditService.updateWidget(Number(id!), state.name!, state.title!, state.type!, state.status!, bannerIds!);
    } catch (error) {
  ...
  function onDelete() {
    ...
    setIsDeleting(true);
    WidgetCreateEditService.deleteWidget(Number(id!));
  }
