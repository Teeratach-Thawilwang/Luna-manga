...
import ConfirmDeleteModal from "@components/backoffice/userRoleDetail/ConfirmDeleteModal";
import UserRoleCreateEditService from "@services/backoffice/UserRoleCreateEditService";
import { createValidateParams, isValid } from "@utils/UserRoleUpdateHelpers";

export default function FooterControl() {
  const { id } = useParams();
  const state = UserRoleCreateEditService.getState();
  ...
  async function onUpdate() {
    ...
    UserRoleCreateEditService.update({ ...validateParams });
    ...
    UserRoleCreateEditService.updateUserRole(Number(id!), state.name!, state.description!, permissionIds, userIds);
  }
  ...
  function onDelete() {
    ...
    setIsDeleting(true);
    UserRoleCreateEditService.deleteUserRole(Number(id!));
  }
