...
export default function InputName({ initial }: { initial?: string }) {
  ...
  const errorMessage = UserRoleCreateEditService.getter<string>("name_error_message");
  const errorMessageElement = getErrorMessageElement(errorMessage);

  useEffect(() => {
    UserRoleCreateEditService.update({ name: name });
  }, [nameDebounce]);
  ...
  return (
    <Box>
      <Title> Name</Title>
      <Input maxLength={50} type="text" placeholder="ชื่อ role" value={name} onChange={onChangeHandle} />
      {errorMessageElement}
    </Box>
  );
}
