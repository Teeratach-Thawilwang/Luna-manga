...
import { createValidateParams, isValid } from "@utils/UserUpdateHelpers";

export default function FooterControl() {
  const state = UserCreateEditService.getState();
  ...
  async function onUpdate() {
    ...
    UserCreateEditService.update({ ...validateParams });
    ...
    setIsCreating(true);
    setPercentBar(100);
    UserCreateEditService.createUser(state.email!, state.password!, state.nick_name!, state.first_name!, state.last_name!, state.status!);
  }
  ...
  return (
    <Box>
      ...
      <CreateButton onClick={onUpdate}>Create</CreateButton>
    </Box>
  );
}
