...
import StorySearchBox from "@components/backoffice/chapterForm/StorySearchBox";
import StoryTable from "@components/backoffice/chapterForm/StoryTable";
...
export default function InputStory({ initial }: { initial?: StoryListInterface }) {
  const story = ChapterCreateEditService.getter<StoryListInterface | null>("story");
  const element = getElement(story);

  useEffect(() => {
    if (initial) {
      ChapterCreateEditService.update({ story: initial });
    }
  }, []);

  const errorMessage = ChapterCreateEditService.getter<string>("story_error_message");
