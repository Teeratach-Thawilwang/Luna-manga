...
export default function InputCoverImage({ initial }: { initial?: ImageInterface }) {
  ...
  const errorMessage = ChapterCreateEditService.getter<string>("cover_image_error_message");
  const errorMessageElement = createErrorMessageElement(errorMessage);

  useEffect(() => {
    if (initial) {
      ChapterCreateEditService.update({ cover_image: initial });
    }
  }, []);
  ...
  function onChangeHandle(event: ChangeEvent<HTMLInputElement>) {
    ...
    ChapterCreateEditService.update({ cover_image: imageFile });
    event.target.value = "";
  }
