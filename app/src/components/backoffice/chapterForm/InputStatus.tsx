...
export default function InputStatus({ initial }: { initial?: ChapterStatusEnum }) {
  const inActiveRef = useRef<HTMLInputElement>(null);
  const activeRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (initial == ChapterStatusEnum.ACTIVE) {
      ChapterCreateEditService.update({ status: ChapterStatusEnum.ACTIVE });
      activeRef.current!.checked = true;
    }
    if (initial == ChapterStatusEnum.INACTIVE || !initial) {
      ChapterCreateEditService.update({ status: ChapterStatusEnum.INACTIVE });
  ...
  return (
    <Box>
      ...
      <CheckBox onClick={() => onCheckBoxClick(inActiveRef)}>
        <input type="checkbox" name={ChapterStatusEnum.INACTIVE} ref={inActiveRef} />
        <Item>Inactive</Item>
      </CheckBox>
      <CheckBox onClick={() => onCheckBoxClick(activeRef)}>
        <input type="checkbox" name={ChapterStatusEnum.ACTIVE} ref={activeRef} />
        <Item>Active</Item>
      </CheckBox>
    </Box>
  );
