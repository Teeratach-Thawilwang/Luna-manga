...
export default function FilterStartDate() {
  ...
  const startDate = ChapterListService.getStartDate();
  const endDate = ChapterListService.getEndDate();
  ...
  function handleChange(date: Date | null) {
    ChapterListService.updateFilter({ start_date: String(date) });
  ...
  return (
    <Box ref={boxRef} onClick={onClickHandle}>
      ...
      />
      <IconBox onClick={() => ChapterListService.updateFilter({ start_date: null })}>
        <CalendarIcon />
      </IconBox>
    </Box>
  );
