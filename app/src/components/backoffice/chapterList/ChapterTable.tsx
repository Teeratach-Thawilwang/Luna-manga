...
import ChapterListPagination from "@components/backoffice/chapterList/ChapterListPagination";
...
export default function ChapterTable() {
  ...
  const chapters = ChapterListService.getData();
  const chapterFilter = ChapterListService.getFilter();
  const isLoading = ChapterListService.getIsLoading();
        ...
        <Tbody>
          {chapters.map((chapter) => (
            <tr key={chapter.id} onClick={() => navigate(`/backoffice/chapter/${chapter.id}`)}>
              ...
              <Td>{transfromDateString(chapter.created_at)}</Td>
              <Td>{transfromDateString(chapter.updated_at)}</Td>
            </tr>
          ))}
        </Tbody>
      </Table>
      <ChapterListPagination navigatePagination={navigatePagination} />
    </Box>
  );
}
