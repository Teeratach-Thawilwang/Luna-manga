...
import { RefreshTokenParams, RefreshTokenResponse, TokenParams, TokenResponse } from "@interfaces/frontside/AuthInterface";
...
class ApiClient {
  ...
  private baseURL: string = import.meta.env.VITE_FRONT_SIDE_API_URL;

  private isRefreshing = false;
  private refreshSubscribers: Array<(token: string) => void> = [];
  private refreshTokenUrl = `${import.meta.env.VITE_FRONT_SIDE_API_URL}/session-refresh`;

  private isGettingToken = false;
  private tokenSubscribers: Array<(token: string) => void> = [];
  private tokenUrl = `${import.meta.env.VITE_FRONT_SIDE_API_URL}/token`;
  ...
  private async responseErrorHandler(error: AxiosError) {
    ...
    const isTokenNotFound = isErrorInData && response.data.error == ResponseErrorEnum.TOKEN_NOT_FOUND;
    const isTokenExpired = isErrorInData && response.data.error == ResponseErrorEnum.TOKEN_EXPIRED;

    if (isNotRetry && isTokenNotFound) {
      return await this.tokenNotFoundHandler(originalRequest);
    }
  ...
  private async tokenNotFoundHandler(originalRequest: InternalAxiosRequestConfig & { isRetry?: boolean }) {
    if (this.isGettingToken) {
      return new Promise((resolve) => {
        this.tokenSubscribers.push((newToken) => {
    ...
    try {
      const params: TokenParams = { client_id: import.meta.env.VITE_FRONTSIDE_CLIENT_ID };
      const { data }: { data: TokenResponse } = await axios.post(this.tokenUrl, params);
      ...
      this.tokenSubscribers.forEach((callback) => callback(data.access_token));
