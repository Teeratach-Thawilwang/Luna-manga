...
import ChapterCommentApi from "@repositories/frontside/ChapterCommentApi";
import AuthService from "@services/frontside/AuthService";
import store, { useAppSelector } from "@store/Store";
import { deleteCommentById, pushData, pushDataAtBegin, update, updateReaction } from "@store/slices/frontside/ChapterCommentSlice";
...
class ChapterCommentService {
  ...
  public getState(): ChapterCommentSliceInterface {
    return useAppSelector((state) => state.frontside.chapterComment, shallowEqual);
  }

  public getComments(): ChapterCommentInterface[] {
    return useAppSelector((state) => state.frontside.chapterComment.data, shallowEqual);
  }

  public getCommentCount(): number {
    return useAppSelector((state) => state.frontside.chapterComment.paginate?.total ?? 0, shallowEqual);
  }

  public getCommentIsLoading(): boolean {
    return useAppSelector((state) => state.frontside.chapterComment.is_loading, shallowEqual);
  }

  public getCommentIsLoaded(): boolean {
    return useAppSelector((state) => state.frontside.chapterComment.paginate != null, shallowEqual);
  }

  public getCommentPaginate(): PaginationInterface | null {
    return useAppSelector((state) => state.frontside.chapterComment.paginate, shallowEqual);
  }

  public getCommentCurrentPage(): number {
    return useAppSelector((state) => state.frontside.chapterComment.paginate?.current ?? 1, shallowEqual);
  }

  public getCommentError(): string {
    return useAppSelector((state) => state.frontside.chapterComment.error, shallowEqual);
  }
