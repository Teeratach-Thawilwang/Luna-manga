...
import { StoryInterface, StoryReactionParams, StoryShowParams, StorySliceInterface } from "@interfaces/frontside/StoryInterface";
import StoryApi from "@repositories/frontside/StoryApi";
import AuthService from "@services/frontside/AuthService";
import store, { useAppSelector } from "@store/Store";
import { update, updateData, updateReaction } from "@store/slices/frontside/StorySlice";
...
class StoryService {
  ...
  public getState(): StorySliceInterface {
    return useAppSelector((state) => state.frontside.story, shallowEqual);
  }

  public getStory(): StoryInterface | null {
    return useAppSelector((state) => state.frontside.story.data, shallowEqual);
  }

  public getIsBookmark(): boolean {
    return useAppSelector((state) => state.frontside.story.data?.is_bookmark ?? false, shallowEqual);
  }

  public getStoryIsLoaded(): boolean {
    return useAppSelector((state) => state.frontside.story.data !== null, shallowEqual);
  }

  public getStoryIsLoading(): boolean {
    return useAppSelector((state) => state.frontside.story.is_loading, shallowEqual);
  }

  public getStoryError(): string {
    return useAppSelector((state) => state.frontside.story.error, shallowEqual);
  }
