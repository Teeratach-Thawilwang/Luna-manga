...
import { GetUserParams, UserInterface, UserSliceInterface } from "@interfaces/backoffice/UserInterface";
import UserApi from "@repositories/backoffice/UserApi";
import AuthService from "@services/backoffice/AuthService";
import store, { useAppSelector } from "@store/Store";
import { update } from "@store/slices/backoffice/UserSlice";
...
class UserService {
  ...
  public getState(): UserSliceInterface {
    return useAppSelector((state) => state.backoffice.user, shallowEqual);
  }

  public getter<T>(key: string): T {
    return useAppSelector((state) => state.backoffice.user.data![key], shallowEqual) as T;
  }

  public getUser(): UserInterface | null {
    return useAppSelector((state) => state.backoffice.user.data, shallowEqual);
  }

  public getIsLoaded(): boolean {
    return useAppSelector((state) => state.backoffice.user.data != null, shallowEqual);
  }
