...
import { CategoryStoryIndexParams, CategoryStoryIndexResponse, CategoryStoryInterface } from "@interfaces/frontside/CategoryStoryInterface";
...
function createStories(params: CategoryStoryIndexParams): CategoryStoryInterface[] {
  ...
  const category = store.getState().frontside.category.data.filter((category) => category.id === params.id)[0] ?? null;
