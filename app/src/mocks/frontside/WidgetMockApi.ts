...
import { BannerIndexResponseInterface, BannerInterface } from "@interfaces/frontside/BannerInterface";
...
class WidgetMockApi {
  ...
  public widgetBanners(params: WidgetsBannersParams, shouldSuccess: boolean = true): ReturnType<WidgetsBannersResponse> {
    return new Promise((resolve, reject) => {
      const widgetState = store.getState().frontside.widget.data.filter((widget) => widget.data.id === params.id)[0] ?? null;
